...
from src.BEANS.interpreter.op_api import check_args, get_args
...
class operation:
    ...
    def execute_operation(mem: tuple[registers, memory], args, pc_index): # type: ignore
        print(f"TEST got called! Argument 1 is num: {check_args(args, ["num"])} with value {get_args(args)}. Raw arguments: {args}")
        return pc_index + 1